...
We begin this section by discussing the experiment by which the appropriate number of time steps to train the agent for was determined.
...
Another parameter to consider when choosing the observation space is the number of LiDAR beams.
...
When racing under evaluation conditions, noise is added to the observation vector to increase the realism of the simulation.
...
Having determined a set of hyper-parameters that yield optimal performance in terms of both safety and lap time for the end-to-end agent racing on the Porto track, we evaluate the agent using Algorithm \ref{alg:end_to_end_deploy}.
Agents trained using the parameters listed in Table \ref{tab:inital_values} demonstrated better performance than any other hyper-parameter set we tested.
...
Ten agents trained with the hyper-parameters listed in Table \ref{tab:inital_values} successfully completed all of their evaluation laps with an average lap time of $6.03$ seconds. 
