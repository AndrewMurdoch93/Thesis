...
\begin{tikzpicture}
    ...
    \draw (3,-0.5) node[shape aspect=1, circle, minimum size=0.1cm, draw] (z1) {};
    ...
    \node (a) at (7.5,-0.5)  [my_style_2] {Scale};
    \node (b) at (7.5,-1.5)  [my_style_2] {Scale};
 
    %Input and output labels
    \matrix (m) at (-0.8,-1) [draw=white, left delimiter={[},right delimiter={]}, matrix of math nodes, row sep=1cm] 
    ...
    \draw[->] (-1.8,-1) -- (-1.4,-1) ; 
    \draw[->] (z1) -- (z1-|a.west)  node [midway, above] (l1) {\small$a_{\text{norm}} \in (-1,1)$} ;      
    \draw[->] (z2) -- (z2-|b.west) node [midway, above] (l2) {\small$\delta_{\text{norm}} \in (-1,1)$} ;               

    \draw[->] (a) -- ($(a)+(1.5,0)$) node [right] (l3) {\small $a_{\text{long},d} \in (\underline{a}, \overline{a})$ }; 
    \draw[->] (b) -- ($(b)+(1.5,0)$) node [right] (l4) {\small $\delta_{d} \in (\underline{\delta}, \overline{\delta})$}; 
