...
\begin{tikzpicture}
    ...
    % \node (d) at (8.5,5.7)  [b] {Velocity\\control};
    ...
    % Label connections
    % \node (l1) at ($(a.east)+(0,0.5)$) [above right] {$v_d$};
    \node (l1) at ($(a.east)+(0,-0.7)$) [above right] {Path};
    \node (l2) at ($(a.east)+(0,0.7)$) [above right] {$a_{\text{long},d}$};
    \node (l3) at (e.east) [above right] {$\delta_{d}$};
    \node (l4) at (c.west) [above left] {Observation};
